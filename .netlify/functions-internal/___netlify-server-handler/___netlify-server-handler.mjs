...
} from '/var/task/workspace/prospecting-tool/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '/var/task/workspace/prospecting-tool/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('/var/task/workspace/prospecting-tool')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '/var/task/workspace/prospecting-tool',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('/var/task/workspace/prospecting-tool/.netlify/dist/run/handlers/server.js')
